    ...
    public RequestObject(String requestURI){
        ...
        if (requestURI.contains("?")){
            ...
            if (uriAndData.length > 1){
                ...
                if (data.contains("&")){
                    //说明有多个参数
                    //{"username=zhangsan","gender=1","interest=food","interest=music"}
                    ...
                    for (String nameAndValue:nameAndValues){
                        ...
                        //1.如果存在，说明该参数为多选框
                        //2.如果不存在，说明为普通的标签
                        if (parameterMap.containsKey(nameAndValueAttr[0])){//多选框
                            //将之前多选框的值取出来
                            String[] values = parameterMap.get(nameAndValueAttr[0]);
                            //定义一个新的数组，新数组的长度比values数组长度大1
                            ...
                            if (nameAndValueAttr.length>1){//说明参数有值
                    ...
                    //通过=进行分割，判断参数是否有值（因为可能存在/oa/user/username=）
